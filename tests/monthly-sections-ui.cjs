...
(async()=>{
  ...
  try {
    ...
    await page.evaluate(()=>{monthlyGoals[currentMonthKey()]=[{text:'월 목표',done:true}];});
    ...
    await page.evaluate(()=>{monthlyGoals[currentMonthKey()]=[{text:'다음 달 목표',done:false}];changeMonth(-1);});
    assert.equal(await page.evaluate(()=>monthlyGoals[currentMonthKey()][0].done),true);
    ...
    await page.evaluate(()=>{monthlyGoals[currentMonthKey()]='기존 원문';renderMonthlyGoals();});
    assert.equal(await page.evaluate(()=>monthlyGoals[currentMonthKey()]),'기존 원문');

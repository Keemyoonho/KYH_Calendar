...
(async()=>{
  ...
  try {
    ...
    await page.addScriptTag({content:`Object.defineProperty(window,'localStorage',{value:{getItem(){return null},setItem(){}}}); window.firebase={initializeApp(){},database(){return {ref(){return {on(){},update(value){window.lastSync=value;return Promise.resolve()}}}}}};`});
    ...
    assert.equal(await page.evaluate(()=>window.lastSync.monthlyGoals),null);
    ...
    assert.equal(await page.locator('#buyList li').count(),5);
    await page.locator('#buyList input[type=checkbox]').first().check();
    await page.getByRole('button',{name:'구매 항목 1 수정',exact:true}).click();
    assert.equal(await page.locator('#buyInput').inputValue(),'물품 tech');
    await page.locator('#buyInput').fill('수정 물품');
    await page.locator('#buyDescription').fill('새 설명');
    await page.locator('#buyCategory').selectOption('medicine');
    await page.locator('#buyPrice').fill('1200');
    await page.getByRole('button',{name:'수정 저장',exact:true}).click();
    assert.equal(await page.locator('#buyList li').count(),5);
    assert.equal(await page.locator('#buyList input[type=checkbox]').first().isChecked(),true);
    assert.match(await page.locator('#buyList li').first().textContent(),/수정 물품.*새 설명.*1,200원/);
    await page.getByRole('button',{name:'구매 항목 1 수정',exact:true}).click();
    await page.locator('#buyInput').fill('취소할 수정');
    await page.getByRole('button',{name:'수정 취소',exact:true}).click();
    assert.match(await page.locator('#buyList li').first().textContent(),/수정 물품/);
    await page.getByRole('button',{name:'구매 항목 1 수정',exact:true}).click();
    await page.evaluate(()=>{loadMonthlySections(window.lastSync);renderBuySlot();});
    ...
    for(const width of [1280,390,320])for(const theme of ['light','dark']){
      ...
      for(const selector of ['#goalTracker','.memo-row'])assert.equal(await page.locator(selector).evaluate(e=>e.scrollWidth<=e.clientWidth),true,`${selector} ${width} ${theme}`);

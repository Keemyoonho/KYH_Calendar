...
(async()=>{
  ...
  try {
    ...
    const migration=await page.evaluate(()=>{
      const old=Array.from({length:5},(_,i)=>({title:'기존 '+i,items:[{text:'물품 '+i,done:true,category:'tech',price:1000}]}));
      const expanded=normalizeBuySlots({buySlots:old});
      return {count:expanded.length,old:expanded.slice(0,5),expected:old,empty:expanded.slice(5)};
    });
    assert.equal(migration.count,10);assert.deepEqual(migration.old,migration.expected);
    assert.ok(migration.empty.every(s=>s.title===''&&s.items.length===0));
    await page.evaluate(()=>goBuySlot(9));
    assert.equal(await page.locator('#buySlotIndicator').textContent(),'10 / 10');
    assert.equal(await page.locator('#buySlotNext').isDisabled(),true);
    await page.locator('#buyInput').fill('열 번째 목록 물품');await page.locator('#buyInput').press('Enter');
    await page.evaluate(()=>pushToFirebase());
    assert.equal(await page.evaluate(()=>window.lastSync.buySlots.length),10);
    assert.equal(await page.evaluate(()=>window.lastSync.buySlots[9].items[0].text),'열 번째 목록 물품');
    await page.evaluate(()=>{buySlots=normalizeBuySlots(window.lastSync);goBuySlot(0);});
    assert.match(await page.locator('.memo-sub-label').first().textContent(),/버킷 리스트/);

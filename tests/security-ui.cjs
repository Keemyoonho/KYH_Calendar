...
(async()=>{
 ...
 try{
  ...
  for(const configured of [false,true]){
   ...
   await page.addScriptTag({content:`window.reads=0;window.writes=0;window.localWrites=0;Object.defineProperty(window,'localStorage',{value:{getItem(){return null},setItem(){window.localWrites++}}});window.fakeAuth={currentUser:null,onAuthStateChanged(cb){window.authCb=cb},setPersistence(){return Promise.resolve()},signOut(){return Promise.resolve()}};window.firebase={initializeApp(){},auth(){return window.fakeAuth},database(){return {ref(){return {off(){},on(event,cb,error){window.reads++;window.dataCb=cb;window.dataError=error},update(){window.writes++;return Promise.resolve()}}}}}};`});

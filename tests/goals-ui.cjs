...
(async()=>{
 ...
 try{
  ...
  await page.locator('.goal-plan-edit').click();
  assert.equal(await page.locator('#goalCreate').textContent(),'수정 저장');
  await page.locator('#goalEditCancel').click();
  assert.equal(await page.evaluate(()=>Object.values(goalPlans)[0].slogan),'꾸준히 성장하기');
  await page.locator('.goal-plan-edit').click();
  await page.locator('#goalSlogan').fill('새로운 슬로건');
  await page.locator('#goalStart').fill('2026-09-10');
  await page.locator('#goalEnd').fill('2026-11-30');
  page.once('dialog',d=>d.accept('전공 문제 5개'));
  await page.getByRole('button',{name:'전공 문제 3개 항목 수정',exact:true}).click();
  await page.getByRole('button',{name:'30분 걷기 추가 취소',exact:true}).click();
  await page.locator('#goalTaskInput').fill('회로 공부');
  await page.locator('#goalTaskAdd').click();
  await page.locator('#goalCreate').click();await page.waitForFunction(()=>!goalPlanSaving);
  ...
  assert.equal(afterEdit.plans[planId].slogan,'새로운 슬로건');
  assert.equal(afterEdit.plans[planId].end,'2026-11-30');
  assert.equal(afterEdit.plans[planId].start,'2026-09-10');
  assert.deepEqual(afterEdit.records,beforeEdit.records);
  const originalTaskId=Object.keys(beforeEdit.plans[planId].items).find(k=>beforeEdit.plans[planId].items[k]==='전공 문제 3개');
  assert.equal(afterEdit.plans[planId].items[originalTaskId],'전공 문제 5개');
  assert.equal(await page.evaluate(()=>goalRows('2026-09-12').find(r=>r.text==='전공 문제 5개').done),true);
  assert.equal(await page.evaluate(()=>goalRows('2026-09-12').find(r=>r.text==='회로 공부').done),false);
  assert.equal(await page.evaluate(()=>goalRows('2026-09-11').some(r=>r.text==='회로 공부')),false);
  assert.equal(await page.evaluate(()=>goalRows('2026-09-11').some(r=>r.text==='30분 걷기')),true);
  assert.equal(await page.evaluate(()=>goalRows('2026-09-12').some(r=>r.text==='30분 걷기')),false);
  await page.locator('.goal-plan-edit').click();
  await page.locator('#goalSlogan').fill('실패할 수정');
  await page.evaluate(()=>{fail=true;});
  await page.locator('#goalCreate').click();await page.waitForFunction(()=>!goalPlanSaving);
  assert.equal(await page.evaluate(()=>Object.values(goalPlans)[0].slogan),'새로운 슬로건');
  assert.equal(await page.locator('#goalSlogan').inputValue(),'실패할 수정');
  await page.evaluate(()=>{fail=false;});
  await page.locator('#goalEditCancel').click();
